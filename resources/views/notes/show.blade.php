            ...
            <div class="flex">
                @if(!$note->trashed())
                    ...
                    <form action="{{ route('notes.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure you want to move this note to the Trash?')">Move To Trash</button>
                    </form>
                @else
                    <p class="opacity-70">
                    ...
                    <form action="{{ route('trashed.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure you want to delete this note permanently?')">Delete Forever</button>
                    </form>
                @endif
            </div>
